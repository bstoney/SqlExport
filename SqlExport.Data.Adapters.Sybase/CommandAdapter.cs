...
namespace SqlExport.Data.Adapters.Sybase
{
    ...
    internal class CommandAdapter : IDbCommandAdapter
    {
        ...
        private IEnumerable<OdbcParameter> GetParameters(string statement)
        {
            using (StringReader sr = new StringReader(statement))
            {
                ...
                while (line != null && (line == string.Empty || line.StartsWith("--:")))
                {
                    if (line != string.Empty)
                    {
                        ...
                        if (param.Length == 3)
                        {
                            OdbcType ot = OdbcType.VarChar;
                            try
                            {
                                ot = (OdbcType)Enum.Parse(typeof(OdbcType), param[1], true);
                            }
                            catch (Exception)
                            {
                            }
                            OdbcParameter op = new OdbcParameter(param[0], ot);
                            op.Value = param[2];
                            yield return op;
                        }
                    }
        ...
        public IEnumerable<IDataResult> ExecuteCommand(TextReader sql)
        {
            ...
            try
            {
                ...
                {
                    foreach (OdbcParameter op in GetParameters(sqlStatement))
                    {
